...
// === KONFIGURATION ===
// Hier trägst du einfach die Namen deiner 7 Bilder ein:
const IMAGE_PATHS = [
...
// Wir berechnen das Raster für den "Texture Atlas" (z.B. 3 Spalten)
const ATLAS_COLS = 3;
...
// Shader Hilfsfunktionen: Baut eine große Collage aus all deinen Bildern
async function createAtlasTexture(imagePaths: string[], slotSize = 512): Promise<THREE.CanvasTexture> {
  ...
  for (let i = 0; i < imagePaths.length; i++) {
    ...
    await new Promise<void>((resolve) => {
      ...
      img.onload = () => {
        ...
        // Bild wird quadratisch zugeschnitten und in das passende Raster-Feld gezeichnet
        ctx.drawImage(
          img, 
      ...
      // Falls ein Bildpfad falsch ist, bricht die Website nicht ab
      img.onerror = () => resolve(); 
  ...
  // Verhindert, dass die Collage auf dem Kopf steht
  texture.flipY = false; 
  return texture;
}
...
export default function ShaderUniverse() {
  ...
  useEffect(() => {
    ...
    // NEU: Welches Bild aus der Collage soll gezeigt werden?
    ...
    while (currentCount < TOTAL_PARTICLES) {
      const size = Math.min(Math.floor(Math.random() * 3) + 1, TOTAL_PARTICLES - currentCount);
    ...
    clusterSizes.forEach((size, clusterIdx) => {
      ...
      for (let i = 0; i < size; i++) {
        ...
        // Weise diesem Partikel ein zufälliges Bild aus dem Array zu (0 bis 6)
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          vTextureIndex = textureIndex; // Gebe Index an Fragment Shader weiter
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Berechne Spalte und Zeile im Atlas
          float col = mod(vTextureIndex, atlasCols);
          float row = floor(vTextureIndex / atlasCols);

          // UV Koordinaten auf das jeweilige Bild anpassen
          vec2 atlasUv = vec2(
            (col + uv.x) / atlasCols,
            (row + uv.y) / atlasRows
          );
    ...
    createAtlasTexture(IMAGE_PATHS).then((tex) => {
    ...
    const tick = (time: number) => {
      ...
      material.uniforms.uIntro.value = Math.min(1, Math.max(0, (elapsedTime - 4.2) * 1.5));
      ...
      for (let i = 0; i < TOTAL_PARTICLES; i++) {
        ...
        const wrappedZ = 25 - ((25 - rawZ) % WRAP_LENGTH + WRAP_LENGTH) % WRAP_LENGTH;

        let currentX = p.x;
        let currentY = p.y;

        // Richtungswechsel-Effekt beim Passieren der Kamera
        if (wrappedZ > 0) {
          const exitFactor = wrappedZ * 12; 
          
          if (p.side === 3) currentY -= exitFactor; // UNTEN abtauchen
          else if (p.side === 0) currentX -= exitFactor; // LINKS wegdriften
          else if (p.side === 1) currentX += exitFactor; // RECHTS wegdriften
          else if (p.side === 2) currentY += exitFactor; // OBEN abdriften
        }

        posAttr.setXYZ(i, currentX, currentY, wrappedZ);

        // 100% Deckkraft überall
        opacAttr.setX(i, 1.0);
      }
